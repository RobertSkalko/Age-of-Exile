...
public class NegativeEffects implements ISlashRegistryInit {
    ...
    public void registerAll() {
        ...
        ExileEffectBuilder.of(BURN, "Burn", EffectType.HARMFUL)
                .spell(Builder.forEffect()
                        .onTick(PartBuilder.justAction(SpellAction.DEAL_DAMAGE.create(ValueCalculationData.base(2), Elements.Fire).put(MapField.DMG_EFFECT_TYPE, EffectData.EffectTypes.DOT_DMG.name())).setTarget(TargetSelector.TARGET.create()).onTick(20D))
                        .onTick(PartBuilder.aoeParticles(ParticleTypes.FLAME, 5D, 1D).onTick(20D))
                        .onTick(PartBuilder.playSound(SoundEvents.BLOCK_CAMPFIRE_CRACKLE, 0.5D, 1D).onTick(20D))
                        .buildForEffect())
                .build();
